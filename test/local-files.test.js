...
test('module hooks lock the inherited typed-array length getter', async (t) => {
  const files = await fixture({
    'entry.mjs': `
      import fs from 'node:fs'
      export default () => {
        ...
        try {
          Object.defineProperty(typedArrayPrototype, 'length', {
            ...descriptor,
            get () { throw new Error('poisoned typed-array length') }
          })
        } catch (error) {
          mutationError = error.name
        }
        const descriptorNumber = fs.openSync(new URL('./value.txt', import.meta.url), 'r')
        fs.closeSync(descriptorNumber)
        return { configurable: descriptor.configurable, mutationError }
      }
    `,
  ...
  assert.deepEqual(await runUntrustedFile(files.path('entry.mjs'), {
    rootDirectory: files.directory,
    timeoutMs: 5_000
  }), { configurable: false, mutationError: 'TypeError' })
})
